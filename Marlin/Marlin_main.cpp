...
void handle_status_leds() {
  if (ELAPSED(millis(), next_status_led_update_ms)) {
    ...
    if (Rval + Gval + Bval == 3) {
      //length of delay
      for (int r = 0; r < LEDmax; r++) {
        Rval = r;
        analogWrite(STAT_LED_RED_PIN, Rval);
        delay(t);
      }
      for (int r = LEDmax; r > 0; r--) {
        Rval = r;
        analogWrite(STAT_LED_RED_PIN, Rval);
        delay(t);
      }
      for (int g = 0; g < LEDmax; g++) {
        Gval = g;
        analogWrite(STAT_LED_GREEN_PIN, Gval);
        delay(t);
      }
      for (int g = LEDmax; g > 0; g--) {
        Gval = g;
        analogWrite(STAT_LED_GREEN_PIN, Gval);
        delay(t);
      }
      for (int b = 0; b < LEDmax; b++) {
        Bval = b;
        analogWrite(STAT_LED_BLUE_PIN, Bval);
        delay(t);
      }
      for (int b = LEDmax; b > 0; b--) {
        Bval = b;
        analogWrite(STAT_LED_BLUE_PIN, Bval);
        delay(t);
      }
      for (int w = 0; w < LEDmax; w++) {
        Rval = w;
        Gval = w;
        Bval = w;
        analogWrite(STAT_LED_RED_PIN, Rval);
